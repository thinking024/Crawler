<%@ page import="controller.VideoCrawler" %>
  ...
  <div class="row clearfix">
    <div class="col-md-12 column">
      <nav class="navbar navbar-expand-lg navbar-light bg-light" style="background-color: #e3f2fd;">
        ...
        <form action="SearchVideo.jsp" class="form-inline  my-2 my-lg-0">
      ...
      %>
      <a class="badge badge-danger" href=<%=hotUrl%> ><%=word.getKeyword()%></a>
      <%
        }
      %>
      热搜up主：
      ...
      <br>
      <%
        if (pageNumber != 0 && videos != null && !(videos.isEmpty())) {
          // 通过href拼接上order参数
      ...
      <table class="table table-hover">
        ...
        <tr>
          <td width="300px"><a style="color:#3CC " href=<%=video.getHref() %>><%=video.getTitle() %>
          </a></td>
          <td width="5%"><%=video.getUploadTime()%>
          </td>
          <td width="1px"><%=video.getPlay()%>
          </td>
          <td width="1px"><%=video.getDanmu()%>
          </td>
          <td width="1px"><%=video.getLength()%>
          </td>
          <td width="1px"><a style="color:#F9C " href=<%=video.getUpUrl()%>><%=video.getUpName()%>
          </a></td>
          <td width="200px"><%=video.getDescription()%>
          </td>
        </tr>
        <%}%>
        </tbody>
